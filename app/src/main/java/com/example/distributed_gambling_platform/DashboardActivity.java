...
public class DashboardActivity extends AppCompatActivity {
    ...
    private boolean searchResultIsValid(ArrayList<String> games) {
    ...
    private ArrayList<ListItem> createListFromSearchResult(ArrayList<String> games) {
        ...
        for (String game : games) {
            ...
            minBet = Float.parseFloat(game.split(":")[1]);
            maxBet = Float.parseFloat(game.split(":")[2]);
            riskLevel = game.split(":")[3];
            bettingCategory = game.split(":")[4];
            stars = Double.parseDouble(game.split(":")[5]);
            jackpot = Float.parseFloat(game.split(":")[6]);
    ...
    private void showRateDialog(ListItem item) {
        ...
        if (rateDialog.getWindow() != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                rateDialog.getWindow().addFlags(WindowManager.LayoutParams.FLAG_BLUR_BEHIND);
                rateDialog.getWindow().getAttributes().setBlurBehindRadius(30);
            } else {
                rateDialog.getWindow().setDimAmount(0.7f);
            }
